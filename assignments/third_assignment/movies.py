...
def main():
    # assigning variables with csv files
    ...
    # reading csv files feat. lil-read_csv
    node = read_csv(nodes)
    edge = read_csv(edges)
    
    # using csv files in functions 
    gen_nm = generate_name_map(node)
    name_e = name_edges(edge,gen_nm)
    movie = generate_movie_dictionary(name_e)
    actor_friends = get_actor_friends(movie)

    print(get_actor_friends("Leonardo DiCaprio"))
